...
class App {
 public:
    App() {
        ...
        // SDL_ttf will generate a RGBA surface, so we must open blend
        glEnable(GL_BLEND);
 ...
 private:
    void initSystem() {
        ...
        // open multi-sample
        SDL_GL_SetAttribute(SDL_GL_MULTISAMPLEBUFFERS, 1);  // set 1 to open multisample
        SDL_GL_SetAttribute(SDL_GL_MULTISAMPLESAMPLES, 2);  // typical values are 2, 4
    ...
    void prepProgram() {
        ...
        loadShader(_vshader, GL_VERTEX_SHADER, "./shaders/11_font.vs");
        SDL_Log("Loaded Vertex Shader");
        loadShader(_fshader, GL_FRAGMENT_SHADER, "./shaders/11_font.fs");
    ...
    void prepTexture() {
        ...
        SDL_Surface* surface = createFontSurface("hello world");
    ...
    SDL_Surface* createFontSurface(string text) {
        TTF_Font* font = TTF_OpenFont("./resources/SimHei.ttf", 48);
        if (!font) {
            SDL_Log("font create failed");
        }
        SDL_Surface* surface = TTF_RenderText_Blended(font, text.c_str(), {0, 255, 0, 255});
        if (!surface) {
            SDL_Log("Font surface faild");
        }
        TTF_CloseFont(font);
        return surface;
    }
    ...
    void setMatrixes() {
        ...
        static const float delta_x = 1, delta_y = 2;
