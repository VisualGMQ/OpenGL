...
class App {
 ...
 private:
    ...
    void prepProgram() {
        ...
        loadShader(_vshader, GL_VERTEX_SHADER, "./shaders/5_depth.vs");
        SDL_Log("Loaded Vertex Shader");
        loadShader(_fshader, GL_FRAGMENT_SHADER, "./shaders/5_depth.fs");
    ...
    // will draw a far rectangle and a near rectangle
    // but the near can't hide the far, because I set the depth function = GL_ALWAYS
    void step() {
    ...
    void setMatrixes(float distance) {
